...
CACHE_FILE = pathlib.Path('.cache/community_stats.json')
...
def get_all_pull_requests_ever(*, update=False):
    ...
    end_cursor = ""
    if pull_requests:
        end_cursor = pull_requests[-1]['cursor']
        if not update:
            return pull_requests  # Early bail out
...
def break_prs_by_core_or_community(*, employees, prs, year=None, month=None, start=None, end=None):
...
def do_it_two():
    with open('.cache/employees.txt') as f:
        employees = set(e.strip() for e in f)
    prs = get_all_pull_requests_ever(force_reload=False)
    prs = [simplify(pr) for pr in prs]
    print('PR Number\tAuthor\tStatus\tCreated At\tClosed At')
    for pr in sorted(prs, key=lambda x: x.number):
        print(f"{pr.number}\t{pr.author}\t{pr.state}\t{pr.created_at}\t{pr.closed_at}")
    return
    merged_prs = [pr for pr in prs if pr.state == 'MERGED']
    closed_prs = [pr for pr in prs if pr.state == 'CLOSED']
    open_prs = [pr for pr in prs if pr.state == 'OPEN']
    ...
    while False and year < this_year or (year == this_year and month <= this_month):
    ...
    #print(f"Start date\tEnd date (inclusive)\t# of PRs opened by community members\t# of unique contributors\t# of PRs opened by current employees\t# of unique employee contributors")
    for start, end in (
        (
            datetime(year=2019, month=10, day=10),
            datetime(year=2020, month=2, day=4),
        ),
        (
        ...
        opened_by_current_employees, opened_by_community = break_prs_by_core_or_community(employees=employees, prs=prs, start=start, end=end+timedelta(days=1))
        checksum += len(opened_by_current_employees) + len(opened_by_community)
        unique_employees = len(set(e.author for e in opened_by_current_employees))
        unique_community = len(set(e.author for e in opened_by_community))
        print(f"{start.date()}\t{end.date()}")
        for pr in sorted(opened_by_community, key=lambda x: x.number):
            print(f"{pr.number}\t{pr.author}")
        print('*'*30)
        for pr in sorted(opened_by_current_employees, key=lambda x: x.number):
            print(f"{pr.number}\t{pr.author}")
        #print(f"{start.date()}\t{end.date()}\t{len(opened_by_community)}\t{unique_community}\t{len(opened_by_current_employees)}\t{unique_employees}")
...
def do_it():  # Shia LeBeouf!
    pull_requests = get_all_pull_requests_ever(update='--update' in sys.argv)
    closed = 0
    active = 0
    for pr in pull_requests:
        if pr['node']['state'] == "OPEN":
            active += 1
        else:
            closed += 1
    print(f"Checksum: {active} open + {closed} closed = {len(pull_requests)} - compare to https://github.com/saltstack/salt/pulls", file=sys.stderr)
    fields = ('number', 'author', 'state', 'created_at', 'closed_at', 'reviews')

    if pull_requests:
        writer = csv.DictWriter(sys.stdout, fieldnames=fields)
        writer.writeheader()
    for pr in sorted((simplify(pr) for pr in pull_requests), key=lambda x: x.number):
        writer.writerow(pr._replace(reviews=len(pr.reviews))._asdict())
